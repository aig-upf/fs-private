...
import itertools
from index import IndexDictionary
from base import Atom, Variable, State
        ...
        for name, inst in init.instantiations.items():
            if isinstance(inst, static.UnarySet):  # A predicate
                ...
                for args in inst.elems:
                    ...
                    facts.append(Atom(Variable(name, args), 1))
            else:  # A function
                for args, val in inst.elems.items():
                    facts.append(Atom(Variable(name, args), val))
        return facts
