...
from .f_expression import parse_term_list
...
def parse_condition_aux(alist, negated):
    ...
    if tag in ("and", "or", "not", "imply"):
        ...
        terms = parse_term_list(alist[1:])
        atom_type = Atom if not negated else NegatedAtom
        x = atom_type(alist[0], terms)
        return x
    if tag == "imply":
...
def parse_literal(alist):  # (i.e. parse atomic formula)
    if alist[0] == "not":
        assert len(alist) == 2
        alist = alist[1]
        atom_type = NegatedAtom
    else:
        atom_type = Atom

    return atom_type(alist[0], parse_term_list(alist[1:]))
