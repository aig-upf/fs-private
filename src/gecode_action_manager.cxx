...
SimpleCSP::ptr GecodeActionManager::createCSPVariables( const Action& a, const ScopedConstraint::vcptr& stateConstraints ) {
	...
	for (VariableIdx var:getAllAffectedVariables(a)) {
		Helper::registerPlanningVariable(*csp, var, CSPVariableType::Output, variables, translator);
	}

	for (auto constraint:a.getConstraints()) {
		Helper::getConstraintTranslator(*constraint)->registerVariables(*csp, translator, constraint, variables);
	}
	
	for (auto effect : a.getEffects()) {
		Helper::getEffectTranslator(*effect)->registerVariables(*csp, translator, effect, variables);
	}
