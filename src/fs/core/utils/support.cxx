...
std::vector<unsigned> compute_managed_symbols(const std::vector<const ActionBase*>& actions, const fs::Formula* goal_formula, const std::vector<const fs::Formula*>& state_constraints) {
	...
	std::vector<unsigned> managed(info.getNumLogicalSymbols(), 0); // To begin with, no symbol needs to be managed
		...
		for (const fs::ActionEffect* effect:action->getEffects()) {
			...
			if (const auto* statevar = dynamic_cast<const fs::StateVariable*>(lhs)) {
				...
				if (info.isPredicate(symbol)) { // We have a fluent predicate symbol
					managed.at(symbol) = (unsigned) true;
				}
			}
			...
			for (const fs::FluentHeadedNestedTerm* fluent:Utils::filter_by_type<const fs::FluentHeadedNestedTerm*>(effect->all_terms())) {
				managed.at(fluent->getSymbolId()) = (unsigned) true;
			}
		}
	}
