...
public:
	...
	const Gecode::IntVar& resolveVariable(const fs::Term* term, const FSGecodeSpace& csp) const;
    const Gecode::BoolVar& resolveReifiedAtomVariable(const fs::AtomicFormula* atom, const FSGecodeSpace& csp) const;

	//! Returns the value of the Gecode CSP variable that corresponds to the given term under the given role, for the given CSP
	object_id resolveValue(const fs::Term* term, const FSGecodeSpace& csp) const;
	
	const Gecode::IntVar& resolveVariableFromIndex(unsigned variable_index, const FSGecodeSpace& csp) const;
    const Gecode::BoolVar& resolveBoolVariableFromIndex(unsigned int variable_index, const FSGecodeSpace& csp) const;
	object_id resolveValueFromIndex(unsigned variable_index, const FSGecodeSpace& csp) const;

	//! Helper to resolve several variables at the same time
	Gecode::IntVarArgs resolveVariables(const std::vector<const fs::Term*>& terms, const FSGecodeSpace& csp) const;
	std::vector<object_id> resolveValues(const std::vector<const fs::Term*>& terms, const FSGecodeSpace& csp) const;
	...
	void updateStateVariableDomains(FSGecodeSpace& csp, const RPGIndex& graph) const;
	void updateStateVariableDomains(FSGecodeSpace& csp, const std::vector<Gecode::IntSet>& domains, bool empty_means_no_constraint = false) const;
	void updateStateVariableDomains(FSGecodeSpace& csp, const std::vector<const Gecode::IntSet*>& domains) const;
	void updateStateVariableDomains(FSGecodeSpace& csp, const State& state) const;
	...
	const Gecode::IntVar& resolveInputStateVariable(const FSGecodeSpace& csp, VariableIdx variable) const;

	//! Returns the value of the CSP variable that corresponds to the given input state variable, in the given CSP.
	object_id resolveInputStateVariableValue(const FSGecodeSpace& csp, VariableIdx variable) const {
