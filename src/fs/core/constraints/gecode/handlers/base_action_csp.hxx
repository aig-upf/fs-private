...
protected:
	...
	void compute_support(FSGecodeSpace* csp, RPGIndex& graph) const;
	...
	void process_solution(FSGecodeSpace* solution, RPGIndex& graph) const;
	
	//!
	void simple_atom_processing(FSGecodeSpace* solution, RPGIndex& graph, AtomIdx tuple, unsigned effect_idx, const PartialAssignment& assignment, const Binding& binding) const;
...
// 	void hmax_based_atom_processing(GecodeSpace* solution, RPGIndex& graph, const Atom& atom, unsigned effect_idx, const PartialAssignment& assignment, const Binding& binding) const;
	...
	std::vector<AtomIdx> extract_support_from_solution(FSGecodeSpace* solution, unsigned effect_idx, const PartialAssignment& assignment, const Binding& binding) const;
	
	
	//! Return the ActionID that corresponds to the current action / action schema, for some given solution
	virtual const ActionID* get_action_id(const FSGecodeSpace* solution) const = 0;
	
	//! Return the action binding that corresponds to the given solution - by default, return an empty binding
	virtual Binding build_binding_from_solution(const FSGecodeSpace* solution) const;
	...
	//!
	void extract_nested_term_support(const FSGecodeSpace* solution, const std::vector<const fs::FluentHeadedNestedTerm*>& nested_terms, const PartialAssignment& assignment, const Binding& binding, std::vector<AtomIdx>& support) const;
};
