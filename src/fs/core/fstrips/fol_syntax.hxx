...
public:
    CompositeTerm(unsigned symbol_id, std::vector<const Term*> subterms) : _symbol_id(symbol_id), _children(std::move(subterms)) {}
...
public:
	CompositeFormula(Connective connective, std::vector<const Formula*> subformulas) : _connective(connective), _children(std::move(subformulas)) {}
