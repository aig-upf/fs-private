    ...
    RungeKutta4::operator()( const State& s, const std::vector<DifferentialEquation>& f_expr, State& next, double H, double factor) const {
        ...
        while ( H > 0.0 ) {
            ...
            for ( unsigned i = 0; i < f_expr.size(); i++ ) {
                ...
                float k1_i = fs::value<float>(k1[i].getValue());
            ...
            for ( unsigned i = 0; i < f_expr.size(); i++ ) {
                float un_i = fs::value<float>(un[i].getValue());
                float k2_i = fs::value<float>(k2[i].getValue());
            ...
            for ( unsigned i = 0; i < f_expr.size(); i++ ) {
                float un_i = fs::value<float>(un[i].getValue());
                float k3_i = fs::value<float>(k3[i].getValue());
            ...
            for ( unsigned i = 0; i < f_expr.size(); i++ ) {
                ...
                float k2_i = fs::value<float>(k2[i].getValue());
                float k3_i = fs::value<float>(k3[i].getValue());
                float k4_i = fs::value<float>(k4[i].getValue());
