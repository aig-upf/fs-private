...
create(const Config& config, FeatureEvaluatorT&& featureset, const FeatureIndex& featidx, SBFWSConfig& conf, const StateModelT& model, BFWSStats& stats) {
	using EngineT = SBFWS<StateModelT, FeatureEvaluatorT, NoveltyEvaluatorT>;
	return std::unique_ptr<EngineT>(new EngineT(model, std::move(featureset), featidx, stats, config, conf));
}
...
SBFWSDriver<StateModelT>::do_search(const StateModelT& model, const Config& config, const std::string& out_dir, float start_time) {
	...
	if (config.getOption<bool>("bfws.extra_features", false)) {
		...
		if (selector.has_extra_features()) {
			...
			auto featureset = selector.select(&featidx);
			return do_search1<FSMultivaluedNoveltyEvaluatorI, FeatureEvaluatorT>(model, std::move(featureset), featidx, config, out_dir, start_time);
		}
	}
	...
	if (indexer.is_fully_binary()) { // The state is fully binary
		...
		return do_search1<FSBinaryNoveltyEvaluatorI, FeatureEvaluatorT>(model, FeatureEvaluatorT(), featidx, config, out_dir, start_time);
		...
		return do_search1<FSMultivaluedNoveltyEvaluatorI, FeatureEvaluatorT>(model, FeatureEvaluatorT(), featidx, config, out_dir, start_time);
...
ExitCode
SBFWSDriver<StateModelT>::do_search1(const StateModelT& model, FeatureEvaluatorT&& featureset, const FeatureIndex& featidx, const Config& config, const std::string& out_dir, float start_time) {
	SBFWSConfig bfws_config(config);
	
	auto engine = create<StateModelT, FeatureEvaluatorT, NoveltyEvaluatorT>(config, std::move(featureset), featidx, bfws_config, model, _stats);
