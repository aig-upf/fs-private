...
AlldiffConstraint::AlldiffConstraint(unsigned arity) 
	: Constraint(arity), min(arity), max(arity), sorted_vars(arity), u(arity)
{}
...
Constraint::Output AlldiffConstraint::filter(const DomainVector& domains) {
	Output res = bounds_consistency(domains);
	if (res == Output::Failure) return res;
	
	invertDomains(domains);
	Output inverted_res = bounds_consistency(domains);
...
Constraint::Output AlldiffConstraint::incrMin(const DomainVector& domains, int a, int b, unsigned i) {
...
Constraint::Output AlldiffConstraint::insert(const DomainVector& domains, unsigned i) {
	Output output = Output::Unpruned;
	u[i] = min[i];
...
Constraint::Output AlldiffConstraint::bounds_consistency(const DomainVector& domains) {
	Output output = Output::Unpruned;
