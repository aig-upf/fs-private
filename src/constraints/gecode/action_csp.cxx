...
static void processVariable( ActionCSP& csp, const ProblemInfo& info, VariableIdx var, IntVarArgs& varArray ) {
...
ActionCSP::ActionCSP( const ProblemInfo& info, const VariableIdxVector& inputVars, const VariableIdxVector& outputVars)
	: _info( info )
{
	IntVarArgs tmpX;
	...
	std::cout << "Created ActionCSP with input variables: " << _X << " and output variables: " << _Y << std::endl;
}
...
void ActionCSP::addEqualityConstraint( VariableIdx v, bool value ) {
	auto it = _inVarsMap.find( v );
	assert( it != _inVarsMap.end() );
	value = ( value ? 1 : 0 );
	// MRJ: v = value
...
void ActionCSP::addEqualityConstraint( VariableIdx v, ObjectIdx value ) {
	auto it = _inVarsMap.find( v );
	assert( it != _inVarsMap.end() );
	//MRJ: v = value
	auto& z = _X[ it->second ];
// 	std::cout << z << std::endl;
	rel( *this, z, IRT_EQ, value );
// 	std::cout << z << std::endl;
}


void ActionCSP::addBoundsConstraint( VariableIdx v, int lb, int ub ) {
	auto it = _inVarsMap.find( v );
	assert( it != _inVarsMap.end() );
	// MRJ: lb <= v <= ub
...
void ActionCSP::addBoundsConstraintFromDomain( VariableIdx v ) {
	auto it = _inVarsMap.find( v );
	assert( it != _inVarsMap.end() );
...
void ActionCSP::addMembershipConstraint( VariableIdx v, DomainPtr varDomain ) {
	auto it = _inVarsMap.find( v );
	assert( it != _inVarsMap.end() );
	// MRJ: v \in dom
	TupleSet valueSet;
...
ActionCSP::ptr ActionCSP::create( const ProblemInfo& info, const Action& a, const ScopedConstraint::vcptr& stateConstraints ) {
...
// 	}
	...
	ActionCSP::ptr csp = new ActionCSP( info, tmpX, tmpY );
