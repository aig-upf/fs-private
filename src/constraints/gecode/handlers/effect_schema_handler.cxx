...
std::vector<std::shared_ptr<BaseActionCSPHandler>> EffectSchemaCSPHandler::create(const std::vector<const ActionSchema*>& schemata, bool approximate, bool novelty) {
...
std::vector<std::shared_ptr<EffectSchemaCSPHandler>> EffectSchemaCSPHandler::create_derived(const std::vector<const ActionSchema*>& schemata, bool approximate, bool novelty) {
	...
	for (auto schema:schemata) {
		for (unsigned eff_idx = 0; eff_idx < schema->getEffects().size(); ++eff_idx) {
			auto handler = std::make_shared<EffectSchemaCSPHandler>(*schema, eff_idx, approximate, novelty);
...
EffectSchemaCSPHandler::EffectSchemaCSPHandler(const ActionSchema& action, unsigned effect_idx, bool approximate, bool novelty)
	: ActionSchemaCSPHandler(action, { action.getEffects().at(effect_idx) }, approximate, novelty)
{}
