...
const DirectEffect* AdditiveTermRhsTranslator::translate(const fs::StateVariable& affected, const fs::Term& rhs) const {
	...
	else if (lhs_var && rhs_const)     return new AdditiveUnaryEffect(lhs_var->getValue(), affected.getValue(), boost::get<int>(rhs_const->getValue()));
	else if (lhs_const && rhs_var)     return new AdditiveUnaryEffect(rhs_var->getValue(), affected.getValue(), boost::get<int>(lhs_const->getValue()));
...
const DirectEffect* SubtractiveTermRhsTranslator::translate(const fs::StateVariable& affected, const fs::Term& rhs) const {
	...
	else if (lhs_var && rhs_const)     return new SubtractiveUnaryEffect(lhs_var->getValue(), affected.getValue(), boost::get<int>(rhs_const->getValue()));
	else if (lhs_const && rhs_var)     return new SubtractiveUnaryEffect(rhs_var->getValue(), affected.getValue(), boost::get<int>(lhs_const->getValue()));
	else throw std::runtime_error("Constant expression should have been compiled earlier");
}
...
const DirectEffect* MultiplicativeTermRhsTranslator::translate(const fs::StateVariable& affected, const fs::Term& rhs) const {
	...
	else if (lhs_var && rhs_const)     return new MultiplicativeUnaryEffect(lhs_var->getValue(), affected.getValue(), boost::get<int>(rhs_const->getValue()));
	else if (lhs_const && rhs_var)     return new MultiplicativeUnaryEffect(rhs_var->getValue(), affected.getValue(), boost::get<int>(lhs_const->getValue()));
	else throw std::runtime_error("Constant expression should have been compiled earlier");
}
