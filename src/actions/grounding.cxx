...
std::vector<GroundAction::cptr> ActionGrounder::ground(const std::vector<ActionSchema::cptr>& schemata, const ProblemInfo& info) {
	...
	for (const ActionSchema::cptr schema:schemata) {
		...
		if (signature.empty()) { 
			...
			id = ground(id, schema, {}, info, grounded);
			++total_num_bindings;
			continue;
		}
		...
		std::cout <<  "Grounding action schema '" << schema->getName() << "' with " << num_bindings << " possible bindings:\n\t" << std::flush;
		...
		for (; !cross_product.ended(); ++cross_product) {
			id = ground(id, schema, Binding(*cross_product), info, grounded);
			++i;
...
unsigned ActionGrounder::ground(unsigned id, ActionSchema::cptr schema, const Binding& binding, const ProblemInfo& info, std::vector<const GroundAction*>& grounded) {
	FDEBUG("grounding", "Binding: " << print::binding(binding, schema->getSignature()));
	GroundAction* ground = schema->bind(id, binding, info);
	if (ground) {
